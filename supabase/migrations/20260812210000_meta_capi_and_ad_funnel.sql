-- Meta CAPI log + view de performance por anúncio (gasto → MQL → ganho)
...
COMMENT ON TABLE public.meta_capi_events_log IS
  'Idempotência e auditoria dos eventos enviados à Meta Conversions API.';

-- Funil CRM cruzado com ads (stage de contact_submissions / leads_geral)
...
COMMENT ON VIEW public.v_meta_ad_crm_funnel IS
  'Leads de ads cruzados com stage CRM: is_mql (funil comercial), avancado (SAL+), converted (ganho).';
